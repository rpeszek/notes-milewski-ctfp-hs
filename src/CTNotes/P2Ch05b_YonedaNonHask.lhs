...
Again, this is not any different than Hask ([N_P2Ch05a_YonedaAndMap](N_P2Ch05a_YonedaAndMap))
...
`Process2` type expresses process with possible trasformations:
```
   Start1 --> Mid1 --->  End2
         \  /\     \  /\
          \/        \/
   ...
   Start2 --> Mid2 ---> End2
```
Implemented CFunctor Process2 provides this mapping of morphisms to these transformations
```
  Start1 --> Mid1 --->  End2
                  \  /\
                   \/
  ...
  Start2 --> Mid2 ---> End2              
```
Trasformations that are not evidenced by the functor are still valid values of the type
so we can construct polymorphic functions that do not play nice with functor subset within Process2.
However we can still 'pick' the good once.

Here is the implementation of the functor repeated:
```
instance CFunctor Process2 HomSet (->) where
   cmap = go  
       where
           step1 :: Process2 'A -> Process2 'B
           step1 P2Start1 = P2Mid1 P2Start1
           step1 P2Start2 = P2Mid2 P2Start2
           go :: HomSet a b -> Process2 a -> Process2 b      
           go MorphId   =  \x -> x
           go MorphAB   = step1 
           go MorphBC1  = P2End1 
           go MorphBC2  = P2End2 
           go MorphAC1  = P2End1 . P1Mid  
           go MorphAC2  = P2End2 . P2Mid 
```
...
Again `forall (x :: A_B__C.Object) . A_B__C.HomSet 'A_B__C.A x -> Process2 x` contains more functions that I will use
in my definition of `cmap`.  This is because there are some bad polymorphic functions that do not play nice with my 
functor.
