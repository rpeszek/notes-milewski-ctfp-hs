...
=======================================================================
This note explores Yoneda Lemma for Functors from non-Hask categories in Haskell. 

It also provides a nice intuition. 
...
Following definition is quoted from `Control.Category` with added explicit kind signature:
```
...
TODO Are Naturality Conditions always automatically true?  Need to investigate non-Hask Natural Transformations  
...
Generalized CYoneda still is automatically always a functor
...
Isomorphism
-----------
Yoneda Lemma states that types `f a` and `CYoneda homset f a` are isomorphic.  
Again, this is not different than Hask ([N_P2Ch05a_YonedaAndMap](N_P2Ch05a_YonedaAndMap))
...
My goal is to explore impact Yoneda makes on construction of `cmap` on such functors.  
These functor `cmap` implementations pattern match on all `A -> B => C` morphisms.
Yoneda is simply reordering that pattern match and grouping it by the functor data constructors! 

Consider for example this example (repeated from [N_P1Ch07b_Functors_AcrossCats](N_P1Ch07b_Functors_AcrossCats), 
not actual code):
```
data Process2 (o :: A_B__C.Object) where
    P2Start1 ::  Process2 'A_B__C.A
    P2Start2 ::  Process2 'A_B__C.A
    P2Mid1   ::  Process2 'A_B__C.A -> Process2 'A_B__C.B
    P2Mid2   ::  Process2 'A_B__C.A -> Process2 'A_B__C.B
    P2End1   ::  Process2 'A_B__C.B -> Process2 'A_B__C.C
    P2End2   ::  Process2 'A_B__C.B -> Process2 'A_B__C.C
  
instance CFunctor Process2 A_B__C.HomSet (->) where
   cmap = go  
       where
           step1 :: Process2 'A_B__C.A -> Process2 'A_B__C.B
           step1 P2Start1 = P2Mid1 P2Start1
           step1 P2Start2 = P2Mid2 P2Start2
           go :: A_B__C.HomSet a b -> Process2 a -> Process2 b      
           go A_B__C.MorphId   =  \x -> x
           go A_B__C.MorphAB   = step1 
           go A_B__C.MorphBC1  = P2End1 
           go A_B__C.MorphBC2  = P2End2 
           go A_B__C.MorphAC1  = P2End1 . step1  
           go A_B__C.MorphAC2  = P2End2 . step1 
```
If I reorder this pattern match and look at value constructors of `Process2 a`, say 
...
We have isomorphism `f a ~= CCoYoneda homset f a`

No good `cmap` construction __co-intuitions__ that I can see.  
...
also see my main notes on Yoneda and map [N_P2Ch05a_YonedaAndMap](N_P2Ch05a_YonedaAndMap).)
