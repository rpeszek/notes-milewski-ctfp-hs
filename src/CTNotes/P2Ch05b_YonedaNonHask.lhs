...
can express Functors between different categories (quote, not actual code):
```
...
there maybe some extra polymorphic functions that fail the naturally test 
...
using category A->B=>C with objects A, B, C and generated by 3 not trivial morphisms MorphAB, MorphBC1, MorphBC2
...
do not play nice with my functor.  For example this is perfectly polymorphic choice not consistent with the functor:
...
also see my main notes on the strong relationship between Yoneda and map in 
...
TODO actual code examples, may need more thinking.
